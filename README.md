...
- A **conversational surface** (`/archangel`) that stores nothing: history lives in the visitor's browser, the server keeps no conversation record, and every verse the model cites is attested against the corpus and shown as verified or "from memory".
- **Deterministic verification by Loom** (`app/loom_bridge.py`): every claim's citations are grounded against the canonical corpus by a truth-maintenance engine; textual support that does not derive is withdrawn automatically, with an inspectable derivation trace.
