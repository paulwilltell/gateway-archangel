...
- **Deterministic verification by Loom** (`app/loom_bridge.py`), in two stages. *Provenance:* a truth-maintenance engine checks every citation is attested in the canonical corpus and withdraws support that does not derive. *Entailment:* the model classifies each (claim, passage) pairing — speech act, audience, covenant scope, whether conditions are kept — and the **published rules** in `app/hermeneutic.py` derive the support level from those classifications, overruling whatever the model asserted. Both produce inspectable traces. This relocates interpretation rather than eliminating it: classifying a passage is still a judgment, but the step from classification to verdict is deterministic, versioned, and published at `/method#hermeneutic`.
